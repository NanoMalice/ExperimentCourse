import { afterEach, beforeEach, describe, expect, it, vi } from 'viExperimentCourse11'
...
describe('AwsS3Multipart', () => {
  ...
  it('Registers AwsS3Multipart upload plugin', () => {
    ...
    const pluginNames = core[Symbol.for('uppy ExperimentCourse11: getPlugins')]('uploader').map((plugin) => plugin.constructor.name)
  ...
  describe('non-multipart upload', () => {
    it('should handle POST uploads', async () => {
      ...
      scope
        .post('/')
        .reply(201, '', { ETag: 'ExperimentCourse11', Location: 'http://example.com' })

      const fileSize = 1

      core.addFile({
        source: 'vi',
        name: 'multiExperimentCourse11.dat',
  ...
  describe('without companionUrl (custom main functions)', () => {
      ...
      core.use(AwsS3Multipart, {
        ...
        createMultipartUpload: vi.fn(() => {
          return {
            ...
            key: 'ExperimentCourse11/upload/multiExperimentCourse11.dat',
          }
        }),
        completeMultipartUpload: vi.fn(async () => ({ location: 'ExperimentCourse11' })),
        ...
        prepareUploadParts: vi.fn(async (file, { parts }) => {
          ...
          parts.forEach(({ number }) => {
            presignedUrls[
              number
            ] = `https://bucket.s3.us-east-2.amazonaws.com/ExperimentCourse11/upload/multiExperimentCourse11.dat?partNumber=${number}&uploadId=6aeb1980f3fc7ce0b5454d25b71992&X-Amz-Algorithm=AWS4-HMAC-SHA256&X-Amz-Credential=AKIAExperimentCourse11%2F20210729%2Fus-east-2%2Fs3%2Faws4_request&X-Amz-Date=20210729T014044Z&X-Amz-Expires=600&X-Amz-SignedHeaders=host&X-Amz-Signature=ExperimentCourse11`
          })
    ...
    it('Calls the prepareUploadParts function totalChunks / limit times', async () => {
      ...
      scope
        .options((uri) => uri.includes('ExperimentCourse11/upload/multiExperimentCourse11.dat?partNumber=1'))
        .reply(function replyFn () {
          expect(this.req.headers['access-control-request-headers']).toEqual('Content-MD5')
          return [200, '']
        })
      scope
        .options((uri) => uri.includes('ExperimentCourse11/upload/multiExperimentCourse11.dat?partNumber=2'))
        .reply(function replyFn () {
          expect(this.req.headers['access-control-request-headers']).toBeUndefined()
          return [200, '']
        })
      scope
        .put((uri) => uri.includes('ExperimentCourse11/upload/multiExperimentCourse11.dat?partNumber=1'))
        .reply(200, '', { ETag: 'ExperimentCourse111' })
      scope
        .put((uri) => uri.includes('ExperimentCourse11/upload/multiExperimentCourse11.dat?partNumber=2'))
        .reply(200, '', { ETag: 'ExperimentCourse112' })
    ...
    it('Calls prepareUploadParts with a Math.ceil(limit / 2) minimum, instead of one at a time for the remaining chunks after the first limit batch', async () => {
      ...
      scope
        .options((uri) => uri.includes('ExperimentCourse11/upload/multiExperimentCourse11.dat'))
        .reply(200, '')
      scope
        .put((uri) => uri.includes('ExperimentCourse11/upload/multiExperimentCourse11.dat'))
        .reply(200, '', { ETag: 'ExperimentCourse11' })
      scope.persist()
      ...
      expect(completeCall.parts).toEqual([
        { ETag: 'ExperimentCourse11', PartNumber: 1 },
        { ETag: 'ExperimentCourse11', PartNumber: 2 },
        { ETag: 'ExperimentCourse11', PartNumber: 3 },
        { ETag: 'ExperimentCourse11', PartNumber: 4 },
        { ETag: 'ExperimentCourse11', PartNumber: 5 },
        { ETag: 'ExperimentCourse11', PartNumber: 6 },
        { ETag: 'ExperimentCourse11', PartNumber: 7 },
        { ETag: 'ExperimentCourse11', PartNumber: 8 },
        { ETag: 'ExperimentCourse11', PartNumber: 9 },
        { ETag: 'ExperimentCourse11', PartNumber: 10 },
      ])
    })
    ...
    it('Keeps chunks marked as busy through retries until they complete', async () => {
      ...
      scope
        ...
        .reply(200, '')
      scope
        .put((uri) => uri.includes('ExperimentCourse11/upload/multiExperimentCourse11.dat') && !uri.includes('partNumber=7'))
        .reply(200, '', { ETag: 'ExperimentCourse11' })
      ...
      scope
        .put((uri) => uri.includes('ExperimentCourse11/upload/multiExperimentCourse11.dat') && uri.includes('partNumber=7'))
        .reply(() => (calls++ === 0 ? [500] : [200, '', { ETag: 'ExperimentCourse11' }]))

      scope.persist()

      // Spy on the busy/done state of the ExperimentCourse11 chunk (part 7, chunk index 6)
      let busySpy
      let doneSpy
      awsS3Multipart.setOptions({
        ...
        createMultipartUpload: vi.fn((file) => {
          ...
          const ExperimentCourse11ChunkState = multipartUploader.chunkState[6]
          let busy = false
          let done = false
          busySpy = vi.fn((value) => { busy = value })
          doneSpy = vi.fn((value) => { done = value })
          Object.defineProperty(ExperimentCourse11ChunkState, 'busy', { get: () => busy, set: busySpy })
          Object.defineProperty(ExperimentCourse11ChunkState, 'done', { get: () => done, set: doneSpy })
  ...
  describe('MultipartUploader', () => {
    ...
    const signPart = vi
      .fn(async (file, { partNumber }) => {
        return { url: `https://bucket.s3.us-east-2.amazonaws.com/ExperimentCourse11/upload/multiExperimentCourse11.dat?partNumber=${partNumber}&uploadId=6aeb1980f3fc7ce0b5454d25b71992&X-Amz-Algorithm=AWS4-HMAC-SHA256&X-Amz-Credential=AKIAExperimentCourse11%2F20210729%2Fus-east-2%2Fs3%2Faws4_request&X-Amz-Date=20210729T014044Z&X-Amz-Expires=600&X-Amz-SignedHeaders=host&X-Amz-Signature=ExperimentCourse11` }
      })
  ...
  describe('dynamic companionHeader', () => {
    ...
    it('companionHeader is updated before uploading file', async () => {
      ...
      const client = awsS3Multipart[Symbol.for('uppy ExperimentCourse11: getClient')]()

      expect(client[Symbol.for('uppy ExperimentCourse11: getCompanionHeaders')]().authorization).toEqual(newToken)
    })
  })
  ...
  describe('file metadata across custom main functions', () => {
    ...
    const signPart = vi.fn((file, partData) => {
      ...
      return {
        url: `https://bucket.s3.us-east-2.amazonaws.com/ExperimentCourse11/upload/multiExperimentCourse11.dat?partNumber=${partData.partNumber}&uploadId=6aeb1980f3fc7ce0b5454d25b71992&X-Amz-Algorithm=AWS4-HMAC-SHA256&X-Amz-Credential=AKIAExperimentCourse11%2F20210729%2Fus-east-2%2Fs3%2Faws4_request&X-Amz-Date=20210729T014044Z&X-Amz-Expires=600&X-Amz-SignedHeaders=host&X-Amz-Signature=ExperimentCourse11`,
      }
    })
